...
int to_logical_pixels(int physical, float device_scale_factor) {
  const float scale = std::max(device_scale_factor, 1.0f);
  return std::max(1, static_cast<int>(static_cast<float>(physical) / scale));
}

void set_windowed_browser_bounds(CefWindowHandle handle, int width, int height,
                                 float device_scale_factor) {
#if defined(__APPLE__)
  ...
  const CGFloat logical_width =
      static_cast<CGFloat>(to_logical_pixels(width, device_scale_factor));
  const CGFloat logical_height =
      static_cast<CGFloat>(to_logical_pixels(height, device_scale_factor));
  [view setFrame:NSMakeRect(0, 0, logical_width, logical_height)];
...
bool wef_init(const WefSettings* wef_settings) {
  ...
  CefRefPtr<WefApp> app(new WefApp(wef_settings->disable_gpu));
...
WefBrowser* wef_browser_create(const WefBrowserSettings* settings) {
  ...
  if (settings->windowless) {
    ...
    CefRect bounds(0, 0,
                   to_logical_pixels(settings->width,
                                     settings->device_scale_factor),
                   to_logical_pixels(settings->height,
                                     settings->device_scale_factor));
    window_info.SetAsChild(reinterpret_cast<CefWindowHandle>(settings->parent),
                           bounds);
  }
  ...
  wef_browser->windowless = settings->windowless;
  wef_browser->deviceScaleFactor = settings->device_scale_factor;
