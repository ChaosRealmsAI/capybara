...
pub fn copy_tokens(
    source_path: &Path,
    project_root: &Path,
) -> Result<CompositionTheme, TimelineError> {
...
fn tokens_css(tokens: &BrandTokens, source_path: &Path) -> Result<String, TimelineError> {
...
fn read_composition(path: &Path) -> Result<CompositionDocument, CompileError> {
    ...
    serde_json::from_str(&text).map_err(|err| {
        CompileError::new(
            ...
            "next step · rerun capy timeline compose-poster",
        )
    })
}
...
fn write_error(path: &Path, err: std::io::Error) -> TimelineError {
    TimelineError::new(
        TimelineErrorCode::OutDirWriteFailed,
