...
pub fn load_tokens(path: &Path) -> Result<BrandTokens, TimelineError> {
    let text = fs::read_to_string(path).map_err(|err| {
        TimelineError::new(
            TimelineErrorCode::BrandTokenMissing,
...
pub fn parse_tokens(path: &Path, text: &str) -> Result<BrandTokens, TimelineError> {
    match extension(path).as_deref() {
        Some("css") => parse_css(text),
        Some("json") => parse_json(text),
        _ => Err(TimelineError::new(
            TimelineErrorCode::BrandTokenMissing,
...
pub fn parse_css(text: &str) -> Result<BrandTokens, TimelineError> {
    let values = text
        .lines()
...
pub fn parse_json(text: &str) -> Result<BrandTokens, TimelineError> {
...
fn empty_error(format: &str) -> TimelineError {
    TimelineError::new(
        TimelineErrorCode::BrandTokenMissing,
