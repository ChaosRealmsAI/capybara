...
pub fn read_poster(path: &Path) -> Result<PosterInput, TimelineError> {
    let text = fs::read_to_string(path).map_err(|err| {
        let code = if err.kind() == std::io::ErrorKind::NotFound {
            TimelineErrorCode::PosterNotFound
        } else {
            TimelineErrorCode::PosterInvalid
        };
        TimelineError::new(
            code,
...
fn invalid_json_error(err: serde_json::Error) -> TimelineError {
    TimelineError::new(
        TimelineErrorCode::PosterInvalid,
        format!("poster JSON is invalid: {err}"),
        "next step · run capy timeline compose-poster --input <poster.json>",
    )
}

fn invalid_poster_error(err: PosterError) -> TimelineError {
    TimelineError::new(
        TimelineErrorCode::PosterInvalid,
