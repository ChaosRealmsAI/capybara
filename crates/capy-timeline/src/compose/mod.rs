...
use crate::error::{ErrorBody, TimelineError, TimelineErrorCode};
...
pub fn compose_poster(req: ComposePosterRequest) -> Result<ComposePosterResult, TimelineError> {
...
pub fn failure(err: TimelineError) -> ComposePosterFailure {
...
fn prepare_project_root(output_dir: PathBuf) -> Result<PathBuf, TimelineError> {
    fs::create_dir_all(&output_dir).map_err(|err| {
        TimelineError::new(
            TimelineErrorCode::OutDirWriteFailed,
            format!(
...
fn write_component(project_root: &std::path::Path) -> Result<PathBuf, TimelineError> {
...
fn write_json<T: Serialize>(path: &std::path::Path, value: &T) -> Result<(), TimelineError> {
    let text = serde_json::to_string_pretty(value).map_err(|err| {
        TimelineError::new(
            TimelineErrorCode::OutDirWriteFailed,
            format!("serialize composition failed: {err}"),
            "next step · rerun capy timeline compose-poster after checking Poster JSON values",
        )
    })?;
    write_text(path, &(text + "\n"))
}

fn write_text(path: &std::path::Path, text: &str) -> Result<(), TimelineError> {
