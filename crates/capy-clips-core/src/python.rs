...
mod tests {
    ...
    fn python_bin_prefers_env_var() -> anyhow::Result<()> {
        ...
        unsafe {
            std::env::set_var("CAPY_CLIPS_CORE_TEST_PYTHON_BIN", "custom-python");
        }
        let resolved = python_bin("CAPY_CLIPS_CORE_TEST_PYTHON_BIN", &preferred);
        // SAFETY: tests serialize environment mutation with `env_lock`.
        unsafe {
            std::env::remove_var("CAPY_CLIPS_CORE_TEST_PYTHON_BIN");
        }
