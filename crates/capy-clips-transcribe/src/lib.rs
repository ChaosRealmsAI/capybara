//! Whisper transcription pipeline for `Capybara clips transcribe`.
...
fn whisper_script_path() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("CAPY_CLIPS_WHISPER_SCRIPT") {
...
mod tests {
    ...
    fn whisper_script_prefers_env_override() -> Result<()> {
        ...
        unsafe {
            std::env::set_var("CAPY_CLIPS_WHISPER_SCRIPT", &script);
        }
        let path = whisper_script_path()?;
        // SAFETY: tests serialize environment mutation with `env_lock`.
        unsafe {
            std::env::remove_var("CAPY_CLIPS_WHISPER_SCRIPT");
        }
