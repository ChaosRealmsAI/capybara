...
#[command(
  ...
  Common commands: init, inspect, design-language inspect, design-language validate, workbench, import-video, clip-queue inspect|write|analyze|semantics|suggest|proposal|proposal-history, generate, campaign, run, add-design, add-artifact.
  Required params: all commands use --project <dir>; generate needs --artifact, --provider, and --prompt; selected target context uses --selector, --json-pointer, or --canvas-node; campaign needs --brief; clip-queue write needs --manifest; clip-queue analyze writes semantic state; clip-queue suggest is read-only; live SDK generation also needs --live; run decisions need a run id.
