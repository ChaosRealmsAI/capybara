...
#[path = "support/timeline_snapshot_cases.rs"]
mod timeline_snapshot_cases;
#[path = "support/timeline_state_cases.rs"]
mod timeline_state_cases;

#[test]
fn timeline_doctor_reports_happy_path_json() -> Result<(), Box<dyn std::error::Error>> {
    let dir = unique_dir("happy")?;
    let recorder = write_fake_binary(&dir, "capy-recorder", "capy-recorder 4.5.6")?;
    ...
    assert_eq!(value["mode"], "crate-only");
    assert_eq!(value["recorder"]["found"], true);
...
#[test]
fn timeline_doctor_reports_missing_json() -> Result<(), Box<dyn std::error::Error>> {
    let output = capy_command()?
        .args([
            "timeline",
            "doctor",
            "--recorder",
            "/definitely/not/capy-recorder",
        ])
        .output()?;
...
#[test]
fn timeline_compose_poster_writes_composition_json() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_compose_poster_writes_brand_tokens() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_compose_poster_reports_invalid_json() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_validate_accepts_composed_composition() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = capy_command()?
        .args(["timeline", "validate", "--composition", composition_path])
        .output()?;
...
#[test]
fn timeline_validate_reports_empty_tracks() -> Result<(), Box<dyn std::error::Error>> {
    let dir = unique_dir("validate-empty")?;
    let composition = dir.join("composition.json");
    fs::write(
        &composition,
        r#"{"schema":"capy.timeline.composition.v1","schema_version":"capy.composition.v1","id":"broken","title":"broken","name":"broken","duration_ms":1000,"duration":"1000ms","viewport":{"w":1920,"h":1080,"ratio":"16:9"},"theme":"default","tracks":[],"assets":[]}"#,
    )?;
...
#[test]
fn timeline_validate_reports_missing_composition() -> Result<(), Box<dyn std::error::Error>> {
    let output = capy_command()?
        .args([
            "timeline",
            "validate",
...
#[test]
fn timeline_validate_rejects_strict_binary_arg() -> Result<(), Box<dyn std::error::Error>> {
    let removed_flag = ["--strict", "-binary"].concat();
    let output = capy_command()?
        .args([
            "timeline",
            "validate",
...
#[test]
fn timeline_compile_writes_render_source_crate() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        value["render_source_schema"],
        "capy.timeline.render_source.v1"
    );
...
#[test]
fn timeline_rebuild_skips_then_recompiles_after_token_change()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let noop = capy_command()?
        .env("PATH", "/definitely/not/on/path")
        .args(["timeline", "rebuild", "--composition", composition_path])
        .output()?;
...
#[test]
fn timeline_compile_reports_missing_composition() -> Result<(), Box<dyn std::error::Error>> {
    let output = capy_command()?
        .env("PATH", "/definitely/not/on/path")
        .args([
            "timeline",
...
#[test]
fn timeline_compile_reports_invalid_composition() -> Result<(), Box<dyn std::error::Error>> {
...
fn timeline_export_writes_mp4_embedded() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = capy_command()?
        .env("CAPY_RECORDER", "/definitely/not/capy-recorder")
        .args([
            "timeline",
            "export",
            "--composition",
...
#[test]
fn timeline_export_reports_missing_render_source() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_verify_export_writes_evidence_index() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_verify_export_reports_missing_composition() -> Result<(), Box<dyn std::error::Error>> {
    let output = capy_command()?
        .args([
            "timeline",
            "verify-export",
