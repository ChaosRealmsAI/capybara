...
type TestServerError = Box<dyn std::error::Error + Send + Sync>;
type TestServerHandle = std::thread::JoinHandle<Result<(), TestServerError>>;

#[test]
fn timeline_state_reports_shell_unavailable_json() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_state_reports_all_attachments_json() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn timeline_state_sends_single_node_query() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = capy_command()?
        .env("CAPYBARA_SOCKET", &socket)
        .args(["timeline", "state", "--canvas-node", "7"])
        .output()?;
    ...
    let handle = std::thread::spawn(
        move || -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            ...
            runtime.block_on(async move {
                ...
                assert_eq!(request["op"], "timeline-state");
                let response = json!({
