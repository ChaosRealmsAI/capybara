...
fn attach_happy_path_records_transition_history() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(value["node"]["kind"], "timeline-composition");
...
fn attach_reports_canvas_node_not_found() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error = match capy_shell::app::timeline::attach_node(
        &state,
        json!({"canvas_node_id": 42, "composition_path": path}),
    ) {
        Ok(value) => return Err(format!("missing node should fail, got {value}").into()),
        Err(error) => error,
    };
...
fn attach_reports_invalid_composition() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error = match capy_shell::app::timeline::attach_node(
        ...
        Ok(value) => return Err(format!("invalid composition should fail, got {value}").into()),
        Err(error) => error,
    };
    let value: Value = serde_json::from_str(&error)?;
...
fn attach_lands_compile_failure_as_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error = match capy_shell::app::timeline::attach_node(
        ...
        Ok(value) => return Err(format!("compile failure should fail attach, got {value}").into()),
        Err(error) => error,
    };
    let value: Value = serde_json::from_str(&error)?;
    let state_value = capy_shell::app::timeline::state_nodes(&state, json!({"canvas_node_id": 0}))?;
...
fn state_nodes_returns_single_attachment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let value = capy_shell::app::timeline::state_nodes(&state, json!({"canvas_node_id": 0}))?;
