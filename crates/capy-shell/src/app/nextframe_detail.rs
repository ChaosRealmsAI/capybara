...
use super::nextframe::AttachedCanvasNode;
use super::nextframe_state::{ExportJob, NextFrameTransition, iso_from_unix};
...
fn compile_detail(composition_path: &Path, history: &[NextFrameTransition]) -> Value {
        ...
        let canvas_node_id = params
            ...
            .ok_or_else(|| {
                detail_error(
                    ...
                    "next step · run capy nextframe state --canvas-node <id>",
                )
            })?;
...
mod tests {
    ...
    use crate::app::nextframe_state::{ExportJob, ExportJobStatus};
    ...
    fn state_detail_reports_single_attachment_chain() -> Result<(), Box<dyn std::error::Error>> {
        ...
        crate::app::nextframe::attach_node(
            &state,
    ...
    fn unique_dir(label: &str) -> Result<PathBuf, std::time::SystemTimeError> {
        Ok(std::env::temp_dir().join(format!(
            "capy-shell-nextframe-detail-{label}-{}-{}",
            std::process::id(),
